...
Six complete, bilingual (Arabic RTL / English LTR) showcase websites that customers can open from the main site. Each has its own identity — palette, typography, layout, navigation and footer — and none of them share DevsHub chrome.
...
`apps/api/src/seed/template-projects.ts` exposes every template as a portfolio project (`template-<slug>`, featured, `liveUrl: /demos/<slug>/ar`, cover `/demos/covers/<slug>-ar.jpg`). Seed v3 (`upgradeToV3`) inserts the missing ones once on an existing database and bumps `settings.seedVersion`; fresh installs get them with the demo data. The web app localises those relative URLs to the visitor's language (`localizeDemoUrl` in `src/lib/localize.ts`), so "Live preview" on `/en` opens the English demo. Requires `SEED_DEMO=true` (the default).
...
4. Add it to `apps/api/src/seed/template-projects.ts` if it should appear in "Our work".
5. Generate covers (below) and run `pnpm test` — `tests/demos.test.mjs` checks registry completeness, ar/en structural parity, bundled artwork and covers.

## Regenerating assets

```bash
pnpm --dir apps/web fonts:demos   # re-download the self-hosted OFL fonts -> src/app/fonts/demos + fonts.local.ts
...
All fonts used by the site and the templates are self-hosted (`src/app/fonts/demos`, OFL) so `next build` never contacts fonts.gstatic.com; `fonts:demos` regenerates them.
