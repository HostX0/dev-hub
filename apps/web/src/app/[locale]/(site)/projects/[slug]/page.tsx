...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: p.coverImage ? [imgUrl(p.coverImage)] : [],
    },
  };
}
...
export default async function ProjectPage({ params }: Props) {
  ...
  const fallbackRelated = related.length
    ? related
    : all.filter((p) => p.id !== project.id).slice(0, 2);
  ...
  return (
    <article className="surface-light relative pt-32 pb-16 md:pt-40">
      <div className="pointer-events-none absolute inset-0 -z-10">
        ...
        <div className="absolute inset-x-0 top-0 h-[700px] bg-[radial-gradient(60%_60%_at_50%_0%,rgba(99,102,241,0.2),transparent_70%)]" />
      </div>
      <div className="container-x">
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
          <div>
            ...
            <Reveal delay={0.1}>
              <h1 className="mt-5 text-3xl font-black leading-[1.25] tracking-tight md:text-5xl">
                {project.title}
              </h1>
            </Reveal>
            {project.tagline && (
          ...
          <Reveal delay={0.1}>
            <dl className="card grid grid-cols-2 gap-5 p-6 sm:grid-cols-3">
              ...
              <div>
                ...
                <dd className="mt-1.5 font-display font-semibold">
                  {project.year ?? "—"}
                </dd>
              </div>
              <div>
                <dt className="flex items-center gap-1.5 text-xs text-muted-2">
                  <Layers className="size-3.5" /> {t.projects.type}
                </dt>
                <dd className="mt-1.5 font-semibold">
                  {categoryLabel(project.category, locale)}
                </dd>
                ...
                <div className="col-span-full border-t border-line pt-4">
                  ...
                  <dd className="mt-2 flex flex-wrap gap-1.5" dir="ltr">
                    {project.tags.map((tag) => (
                      <span
                        key={tag}
                        className="rounded-md border border-line bg-white/[0.03] px-2 py-0.5 font-display text-[11px] text-muted"
                      >
                        {tag}
                      </span>
                    ))}
