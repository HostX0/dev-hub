...
export default function LoginPage() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await clientApi<{ token: string }>("/auth/login", {
        method: "POST",
        json: data,
      });
      auth.set(res.token);
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-full bg-fg font-semibold text-bg transition-all hover:bg-white hover:shadow-glow disabled:opacity-60"
          >
