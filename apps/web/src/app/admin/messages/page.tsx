...
export default function AdminMessages() {
  ...
  async function open(m: Message) {
    setActive(m);
    if (!m.read && can("messages:write")) {
      try {
        await clientApi(`/messages/${m.id}/read`, { method: "PATCH" });
        setList(
          (l) => l?.map((x) => (x.id === m.id ? { ...x, read: true } : x)) ?? l,
        );
      } catch (e) {
              ...
              {list.map((m) => (
                <li key={m.id}>
                  <button
                    onClick={() => open(m)}
                    className={cn(
                      "w-full px-4 py-3 text-start transition-colors hover:bg-white/[0.03]",
                      active?.id === m.id && "bg-brand/10",
                    )}
                  >
            ...
            {!active ? (
              <div className="grid h-full min-h-[260px] place-items-center text-sm text-muted">
                <span className="flex flex-col items-center gap-2">
                  <Mail className="size-8 text-muted-2" /> اختر رسالة لعرضها
                </span>
              </div>
            ) : (
              <div>
                <div className="flex items-start justify-between gap-4 border-b border-line pb-4">
                  ...
                  <div className="flex gap-1">
                    <a
                      ...
                      className="inline-flex h-9 items-center gap-2 rounded-full border border-line px-3 text-xs font-semibold hover:border-brand/50"
                    >
                    ...
                    {can("messages:write") && (
                      <button
                        ...
                        onClick={() => setDel(active)}
                        className="grid size-9 place-items-center rounded-full border border-line text-muted hover:border-danger/50 hover:text-danger"
                      >
                ...
                <p className="mt-5 whitespace-pre-wrap leading-[1.9]">
                  {active.body}
                </p>
              </div>
            )}
