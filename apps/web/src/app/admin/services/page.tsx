...
export default function AdminServices() {
  ...
  async function save() {
    ...
    try {
      ...
      if (editing.id)
        await clientApi(`/services/${editing.id}`, {
          method: "PUT",
          json: payload,
        });
  ...
  const setD = <K extends keyof Draft>(k: K, v: Draft[K]) =>
    setEditing((s) => (s ? { ...s, draft: { ...s.draft, [k]: v } } : s));
    ...
    <div>
      <PageHeader
        ...
        actions={
          <button
            ...
            className="inline-flex h-10 items-center gap-2 rounded-full bg-fg px-4 text-sm font-semibold text-bg"
          >
      ...
      {editing && d && (
        <Card className="mb-6 space-y-5 border-brand/40">
          ...
          <div className="grid gap-4 md:grid-cols-[1fr_200px_100px]">
            ...
            <Field label="الأيقونة">
              <div className="flex items-center gap-2">
                <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-brand/15 text-brand-2">
                  <ServiceIcon name={d.icon} className="size-5" />
                </span>
                <Select
                  value={d.icon}
          ...
          <Field label="المميزات" hint="Enter لإضافة عنصر">
            <TagsInput
              value={d.features}
          ...
          <div className="space-y-4 rounded-xl border border-brand/25 bg-white/[0.02] p-4">
            ...
            <Field lang="en" label="Title">
              <Input
                value={d.titleEn}
                onChange={(e) => setD("titleEn", e.target.value)}
                dir="ltr"
            ...
            <Field lang="en" label="Features" hint="Enter to add an item">
              <div lang="en" dir="ltr">
                <TagsInput
                  value={d.featuresEn}
                  onChange={(v) => setD("featuresEn", v)}
                  placeholder="Feature..."
                />
              </div>
            </Field>
          ...
          <div className="space-y-4 rounded-xl border border-brand/25 p-4">
            <h3 className="text-sm font-bold">
            ...
            <Field lang="ckb" label="ناونیشان">
              <Input
                ...
                dir="rtl"
                value={d.titleCkb ?? ""}
                onChange={(e) => setD("titleCkb", e.target.value)}
              />
            </Field>
            <Field lang="ckb" label="وەسف">
              <Textarea
                lang="ckb"
                dir="rtl"
                value={d.descriptionCkb ?? ""}
                onChange={(e) => setD("descriptionCkb", e.target.value)}
                rows={3}
              />
            </Field>
            <Field lang="ckb" label="تایبەتمەندییەکان">
              <div lang="ckb" dir="rtl">
                <TagsInput
                  value={d.featuresCkb ?? []}
                  onChange={(v) => setD("featuresCkb", v)}
                />
              </div>
            </Field>
          </div>
          <div className="flex justify-end">
            <button
              ...
              className="inline-flex h-10 items-center gap-2 rounded-full bg-fg px-5 text-sm font-semibold text-bg disabled:opacity-60"
            >
              {busy ? (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {list.map((s) => (
            <Card key={s.id} className="flex gap-4">
              <span className="grid size-11 shrink-0 place-items-center rounded-xl bg-brand/15 text-brand-2">
                <ServiceIcon name={s.icon} className="size-5" />
              </span>
              <div className="min-w-0 flex-1">
                <h3 className="font-bold">{s.title}</h3>
                {s.titleEn && (
                  <p className="font-display text-xs text-muted-2" dir="ltr">
                    {s.titleEn}
                  </p>
                )}
              ...
              <div className="flex shrink-0 flex-col gap-1">
                <button
                  onClick={() => setEditing({ id: s.id, draft: toDraft(s) })}
