      ...
      >
        ...
        <div className="flex flex-1 flex-col p-6">
          ...
          {project.tags?.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-1.5" dir="ltr">
              {project.tags.slice(0, 5).map((tag) => (
                <span
                  key={tag}
                  className="rounded-md border border-line bg-white/[0.03] px-2 py-0.5 font-display text-xs text-muted"
                >
                  {tag}
                </span>
              ))}
          ...
          <div className="mt-auto flex items-center justify-between gap-2 pt-5">
            <Link
              href={href}
              className="group/l inline-flex items-center gap-1.5 text-sm font-semibold text-fg transition-colors hover:text-brand-2"
            >
              {t.work.details}
