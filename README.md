...
ستة مواقع كاملة بلغتين (عربي RTL / إنجليزي LTR) بتصاميم مستقلة يستعرضها الزبون من الموقع الرئيسي: **شركة** (`company`)، **محامي** (`lawyer`)، **مصوّر** (`photographer`)، **مطعم** (`restaurant`)، **عيادة** (`clinic`)، **عقارات** (`realestate`). تظهر أيضاً ضمن «أعمالنا» في الرئيسية كمشاريع بزر معاينة مباشرة (تُزرع تلقائياً عبر seed v3 في الـ API).
